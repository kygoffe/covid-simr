  ...
  while (nrow(cal)>0 & tx<max(res$time)) {
    ...
    if (cal$event[ind]=="arrival") {
      ...
      if (occ<cap) {
        ...
        los<-rlnorm(1,meanlog=meanlog,sdlog=sdlog)
    ...
    #NEED TO FILL IN VALUES WHERE OCC IS WRONGLY LEFT AT ZERO BECAUSE NO EVENT HAPPENED THAT DAY ####
...
clusterExport(cl=cl,
              varlist=c("uncert","inputs","meanlog","sdlog","cap","pfat"),
              envir=environment())
...
filename_ext<-paste0("_los_med=",los_median,"_los_q95th=",los_95,"_cap=",cap,"_pfat=",pfat,"_tol=",tol,"_nreps=",nreps)
